"""Tests for the ArcticEmbedder Ollama HTTP client.

Mirrors the Qwen3 embedder test structure — same ``httpx.MockTransport``
plumbing — so coverage of the shared ``_OllamaEmbedderBase`` stays consistent
...
from brain.embeddings import ArcticEmbedder, OllamaEmbedError


def _transport_returning(
    payload: dict[str, object], *, captured: list[bytes] | None = None
) -> httpx.MockTransport:
    """Mock transport returning ``payload`` as JSON for any /api/embed POST."""
...
def test_arctic_embedder_dim_is_1024() -> None:
    """Arctic v2 ships at 1024 native dims — assert the class attribute."""
    assert ArcticEmbedder.dim == 1024
    emb = ArcticEmbedder(host="http://x", client=_client(_transport_returning({})))
    assert emb.dim == 1024


def test_embed_documents_no_prefix() -> None:
    """Documents are sent verbatim — Snowflake's guidance is "prefix only the query"."""
    captured: list[bytes] = []
    transport = _transport_returning(
        {"embeddings": [[0.1] * 1024, [0.2] * 1024]}, captured=captured
    )
    emb = ArcticEmbedder(host="http://x", client=_client(transport))
    out = emb.embed(["doc1", "doc2"], input_type="document")
    assert len(out) == 2
    assert len(out[0]) == 1024
    body = json.loads(captured[0])
    assert body["input"] == ["doc1", "doc2"]
    assert body["model"] == "snowflake-arctic-embed2"


def test_embed_query_prepends_arctic_prefix() -> None:
    """Queries get ``"query: "`` prepended per Snowflake's HF model card."""
    captured: list[bytes] = []
    transport = _transport_returning(
    ...
    assert formatted == "query: what did person-a say?"
...
def test_network_error_raises_ollama_embed_error() -> None:
    def handler(request: httpx.Request) -> httpx.Response:
...
def test_5xx_raises_with_body() -> None:
    def handler(request: httpx.Request) -> httpx.Response:
...
def test_response_with_too_few_embeddings_raises() -> None:
    """Length mismatch → OllamaEmbedError, not silent truncation."""
    transport = _transport_returning({"embeddings": [[0.1] * 1024]})
    emb = ArcticEmbedder(host="http://x", client=_client(transport))
    with pytest.raises(OllamaEmbedError, match="1 embeddings for 2 inputs"):
        emb.embed(["doc1", "doc2"], input_type="document")


def test_malformed_json_raises_ollama_embed_error() -> None:
