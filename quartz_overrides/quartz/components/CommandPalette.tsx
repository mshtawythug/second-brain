// Brain command palette component (Lane C, Cmd/Ctrl-K).
//
...
// in stock Quartz exposes a Cmd-K palette. We render the palette
...
// owns the open/close lifecycle, fuzzy search, keyboard handling,
// and SPA-nav cleanup.
//
...
// `_cmdk.scss` (Lane C SCSS partial) and the DOM walk inside
...
// `../styles/brain/_cmdk.scss`. Don't let them spill into each
// other.
...
const CommandPalette: QuartzComponent = (_props: QuartzComponentProps) => {
  ...
  // selector hook used by `commandPalette.inline.ts` and `_cmdk.scss`
...
// `_cmdk.scss`, which is `@use`-imported via `custom.scss`. That
